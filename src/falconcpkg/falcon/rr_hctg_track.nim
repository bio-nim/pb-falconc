...
proc stream_get_rid_to_ctg(ss: streams.Stream, fn = "rid_to_ctg"): auto =
    ...
    while parsecsv.readRow(csv):
        ...
        if (not global_logged_rid_to_ctg) and (ctg in rid_to_ctg[rid]):
            ...
            log("In ", $fn, ", found dup ctg ", $ctg, " for rid ", $rid,
                    " from Row ", $csv.row, " matching one of ",
                    $sequtils.toSeq(rid_to_ctg[rid].items()))
...
proc get_rid_to_phase(phased_reads_fn, rawread_ids_fn: string): auto =
    ...
    log("Reading phased_reads file: len=", sys.filelength(fn0), " for file '",
            fn0, "'")
    let
    ...
    log("Reading rawread_ids file: len=", sys.filelength(fn1), " for file '",
            fn1, "'")
    #let
...
proc tr_stage1(la4falcon_stream: streams.Stream, fn: string, min_len,
        bestn: int): myprioritytable =
                ...
                if t_phase.blockn != -1:
                    ...
                    if q_id_int < len(global_rid_to_phase[]):
                        ...
                        if q_phase.ctg_id != "":
                            #log("   t_phase:", t_phase)
                            #log("   q_phase:", q_phase)
                            if (q_phase.ctg_id == t_phase.ctg_id and
                                q_phase.blockn == t_phase.blockn and
...
proc run_tr_stage1(db_fn, fn: string, min_len, bestn: int): string =
    ...
    let options: set[osproc.ProcessOption] = {osproc.poUsePath,
            osproc.poEvalCommand, osproc.poEchoCmd}
...
proc run_track_reads*(settings: Settings) =
    ...
    let rid_to_phase = get_rid_to_phase(settings.phased_read_file,
            settings.rawread_ids)
    ...
    for i in 0..<nsubs:
        ...
        let fv: FlowVar[string] = threadpool.spawn spawned_tr_stage1(
                settings.db_fn, fn, settings.min_len, settings.bestn)
        responsesT[i] = fv
        responsesB[i] = fv
    threadpool.sync()
    #while nsubs > 0:
...
proc run_stage2*(
    ...
    var bread_to_areads: myprioritytable = tables.initTable[string,
            binaryheap.Heap[mytuple]](1024)
  ...
  # For each b-read, we find the best contig map throgh the b->a->contig map.
    for bread in algorithm.sorted(sequtils.toSeq(tables.keys(bread_to_areads))):
        ...
        for ctg, score_count in ctg_score.pairs:
            ...
            out_f.writeLine(bread, " ", ctg, " ", score_count.count, " ", rank,
                    " ", score_count.score, " ", in_ctg)
            rank += 1
    #log("spinning...")
    #while true:
    #  discard
