...
var readsToFilter = initTable[int, int]()

proc parseOvl*(s: string): overlap =
    var ovl: overlap
    let ld = s.split(" ")
    doAssert ld.len() == 13
    ovl.ridA = parseInt(ld[0])
    ovl.ridB = parseInt(ld[1])
...
minIdt: float) =
    ...
    var containedBreads = initHashSet[int]()
    var aReadPass = true
...
proc runner*(db: string,
 ...
 filterLog: string,
 outputFn: string) =
    ##Runs the multi-stage m4 overlap filter. In stage one, reads that trigger a
 ##filter are marked including containment, gaps in coverage along the A-read,
 ##and repeat reads. In stage two the filters are applied and the N-best
 ##overlaps are kept for the 5prime and 3prime of each read.
    ...
    for las in js:
        inc(counter)
        stage1.add(
                        "time LA4Falcon -mo {db} {las.getStr()} | falconc overlaps-stage1 --minIdt {idtStage1} {gapFiltOpts} --minLen {minLen} --minCov {minCov} --maxCov {maxCov} --maxDiff {maxDiff} --blacklist {counter}.stage1.tmp.msgpck".fmt)
        stage2.add(
                        "time LA4Falcon -mo {db} {las.getStr()} | falconc overlaps-stage2 --minIdt {idtStage2} --bestN {bestN}  --blacklistIn merged_blacklist.stage1.msgpck --filteredOutput {counter}.tmp.ovl".fmt)
    ...
    let merge1 = execCmd(
                    ...
                    "falconc overlaps-merge-blacklist -b blacklist_msgpck.stage1.fofn -o merged_blacklist.stage1.msgpck")
    if merge1 != 0:
