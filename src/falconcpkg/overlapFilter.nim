 ...
 ##N-best overlaps are kept for the 5prime and 3prime of each read.
    ...
    for ms in m4s:
        ...
        stage1.add(
                                     ...
                                     "cat {ms} | falconc m4filt-stage1 --minIdt {idtStage1} {gapFiltOpts} --minLen {minLen} --minCov {minCov} --maxCov {maxCov} --maxDiff {maxDiff} --blacklist {counter}.stage1.tmp.msgpck".fmt)
        stage2.add(


                                     "cat {ms} | falconc m4filt-stage2 --minIdt {idtStage2} --bestN {bestN}  --blacklistIn merged_blacklist.stage1.msgpck --filteredOutput {counter}.tmp.ovl".fmt)
        ovls.add("{counter}.tmp.ovl".fmt)
        msgpckFofnS1.writeLine("{counter}.stage1.tmp.msgpck".fmt)
    close(msgpckFofnS1)
